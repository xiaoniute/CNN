...
from cp_module import Conv2d, Sigmoid, MaxPool2d, AvgPool2d, Linear, ReLU, CrossEntropyLoss,flatten
import struct
import os 
import glob
...
if __name__ == '__main__':
    ...
    train_images = train_images.astype(cp.float16) / 256
    test_images = test_images.astype(cp.float16) / 256
    ...
    for _ in range(epoch):
        ...
        for i in tq:
            ...
            if (i+1)*batch_size<N:
                ...
                random_integer = cp.random.randint(i*batch_size, (i+1)*batch_size,1)  
            else:
                random_integer = cp.random.randint(i*batch_size, N,1)
        ...
        #--------------------验证---------------------
        random = cp.random.randint(0, test_images.shape[0], batch_size) 
        batch_images = test_images[random]
        batch_lable = test_labels[random]
        out = net.forward(batch_images)
        loss,dx = lossfunction(out , batch_lable)
        max_indices = cp.argmax(out, axis=1)
        num_matches = 0
