...
class Conv2d:
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, 
        ...
        self.x = None
        # 初始化权重和偏置  
        self.weight = np.random.normal(loc=0.0, scale=1.0, size=(out_channels, in_channels, kernel_size, kernel_size))  
        ...
        #初始化梯度
        self.w_grad = np.zeros(shape=(out_channels, in_channels, kernel_size, kernel_size))
        self.b_grad = np.zeros((out_channels, 1))
    ...
    def forward(self, x):
        ...
        for i in range(N):
            ...
            out[i] = (np.dot(image_col,kernel)+self.bias.T).reshape(H_out,W_out,O).transpose(2,0,1) #这里利用1-32的，16*2的数组进行了验证
        ...
        #                 # 选取卷积核计算解释：
        #                 # 每个输出通道对应一个卷积核 
        #                 out[i, f, j, k] = np.sum(window * self.weight[f, :, :, :]) + self.bias[f]  
    ...
    def backward(self, dy, lr):
        ...
        k_180 = np.rot90(self.weight, 2, (2,3))      # numpy矩阵旋转180度
        #填充dy
        pad = WH-1
        ...
        for i in range(N):
            ...
            dx[i] += np.dot(delta_col, kernel).reshape(H,W,C).transpose(2,0,1)
        dx /= N
...
#         return np.tanh(x)
#     def backward(self, dy):
#         return dy * (1 - np.tanh(self.forward(dy)) ** 2)  
...
class AvgPool2d:
    ...
    def backward(self, dy):
        ...
        dx_padding = np.zeros_like(self.x)  
...
class CrossEntropyLoss:
    def __call__(self, x, label):
        ...
        correct_logprobs = -np.log(probs[range(N), label])  
